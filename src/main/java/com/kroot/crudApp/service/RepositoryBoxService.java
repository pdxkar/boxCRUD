...
import com.kroot.crudApp.repository.BoxRepository;
...
public class RepositoryBoxService implements BoxService {
    ...
    public Box create(BoxDTO created) {
     ...
     //   Box box = Box.getBuilder(created.getBoxType()).build();
        Box box = Box.getBuilder(created.getBoxType(), created.getAttributes()).build();
